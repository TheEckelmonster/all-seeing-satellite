...
function Log.set_log_level(new_log_level)
    ...
    if (not is_number(new_log_level) and not is_string(new_log_level) and not new_log_level.valid) then return
        default_return_val() end
...
function log_message(message, log_level, traceback)
    ...
    if (not _log_level or not _log_level.valid) then
        log_print_message("Log level was invalid",
            { level = Log_Constants.levels[Log_Constants.NONE.num_val], valid = true })
        return
    end
    ...
    if (    log_level
        ...
        and log_level.level.num_val >= _log_level.level.num_val
    ) then
